-- FIX: Accepting a connection request was implemented as 3 separate,
-- unguarded client-side REST calls (update connections -> insert threads ->
-- insert thread_members), each independently subject to RLS. A failure on
-- the 2nd or 3rd step leaves the connection marked 'accepted' with no
-- working chat thread, and no way to retry through the UI (the "Received"
-- tab only shows connections still in 'requested' state). connections also
-- has no thread_id column, so there's no reliable way to find "the" thread
-- for a given connection even when one does exist.
--
-- This replaces that flow with a single atomic, idempotent, SECURITY
-- DEFINER function: it validates the caller is a party to the connection,
-- marks it accepted, finds-or-creates the DM thread, links it back onto
-- the connection row, and ensures both parties are thread members -- all
-- in one transaction. Safe to call again on an already-accepted connection
-- to repair/complete it.
