-- Safety infrastructure: blocking and reporting. This app's whole premise
-- is meeting strangers for real-world activities (treks, flatmate
-- searches, meetups) -- a physical-safety category, not just a social
-- feed. Before real strangers can message each other, there needs to be
-- at minimum: a way to block someone, a way to report someone, and a way
-- for the operator to see and act on reports. Reuses the existing
-- app_role/has_role('admin') system already in the schema rather than
-- inventing a new one.
...
-- Enforcement: blocked parties can no longer message each other, even in
-- an existing thread, and can no longer form a new connection.
